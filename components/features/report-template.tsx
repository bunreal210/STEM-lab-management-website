  ...
  return (
    <div id={id} className="hidden bg-white text-slate-900 font-sans p-10" style={{ width: '1000px', minHeight: '1414px' }}>
      ...
      <div className="space-y-12">
        ...
        {showDevices && (
          <section>
            <h2 className="text-2xl font-bold font-heading text-slate-800 mb-4 border-l-4 border-emerald-500 pl-3">
              Tình Trạng Thiết Bị Hiện Tại ({devices.length})
            </h2>
            {devices.length === 0 ? (
              <p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">Không có dữ liệu thiết bị trong hệ thống.</p>
            ) : (
        ...
        {showLoans && (
          <section>
            <h2 className="text-2xl font-bold font-heading text-slate-800 mb-4 border-l-4 border-blue-500 pl-3">
              Nhật Ký Mượn / Trả ({loans.length})
            </h2>
            {loans.length === 0 ? (
              <p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">Không có dữ liệu mượn trả trong khoảng thời gian đã chọn.</p>
            ) : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-600">
        ...
        {showReports && (
          <section>
            <h2 className="text-2xl font-bold font-heading text-slate-800 mb-4 border-l-4 border-rose-500 pl-3">
              Nhật Ký Báo Hỏng / Sự Cố ({reports.length})
            </h2>
            {reports.length === 0 ? (
              <p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">Không có dữ liệu báo hỏng sự cố trong khoảng thời gian đã chọn.</p>
            ) : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-600">
        ...
        {showSchedules && (
          <section>
            <h2 className="text-2xl font-bold font-heading text-slate-800 mb-4 border-l-4 border-purple-500 pl-3">
              Thống Kê Tiết Dạy / Lịch Hoạt Động ({schedules.length})
            </h2>
            {schedules.length === 0 ? (
              <p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">Không có dữ liệu lịch hoạt động trong khoảng thời gian đã chọn.</p>
            ) : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-600">
                ...
                <tbody>
                  {schedules.map(s => (
                    <tr key={s.id} className="border-b border-slate-200">
                      <td className="p-3 border border-slate-200">{s.date}</td>
        ...
        {/* TEACHER FEEDBACK & JOURNALS SECTION */}
        {showTeacherFeedback && (
          <section>
            <h2 className="text-2xl font-bold font-heading text-slate-800 mb-4 border-l-4 border-amber-500 pl-3">
              Nhật Ký Phòng Lab & Đánh Giá Tiết Học ({journals.length})
            </h2>
            {journals.length === 0 ? (
              <p className="text-sm text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-slate-100">Không có dữ liệu nhật ký phòng Lab trong khoảng thời gian đã chọn.</p>
            ) : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-600">
                    <th className="p-3 border border-slate-200">Ngày</th>
                    <th className="p-3 border border-slate-200">Giờ học</th>
                    <th className="p-3 border border-slate-200">Lớp / Loại</th>
                    <th className="p-3 border border-slate-200">Môn học / Tiêu đề</th>
                ...
                <tbody>
                  {journals.map(j => (
                    <tr key={j.id} className="border-b border-slate-200">
                      <td className="p-3 border border-slate-200">{j.date || new Date(j.created_at).toLocaleDateString('vi-VN')}</td>
                      <td className="p-3 border border-slate-200 font-mono text-xs">{format24hTime(j.time_of_day) || '-'}</td>
                      <td className="p-3 border border-slate-200 font-bold">{j.target_class || j.type}</td>
                      <td className="p-3 border border-slate-200">{j.subject || j.title}</td>
